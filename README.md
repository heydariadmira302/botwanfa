...
玩家上下分、期号查账、群排行榜、数据报表、发送队列、操作日志、备份恢复、测试模式和玩法说明。

真人用户加入群时会立即自动建档并创建余额为 `0.00` 的独立群钱包。给新人上分时，超级管理员
私聊机器人，依次选择 `玩家上下分 -> 目标群 -> 新人 -> 上分`，输入“金额 备注”并二次确认。
升级前已经存在的钱包余额保持原值，不会被重置。
...
- 在“期号查账”中粘贴 32 位期号或整条开奖消息，查询该期全部玩家的逐项下注金额、当期倍率、输赢、返还、净输赢、奖励和结后余额。
