...
4. 默认邀请本期累计有效下注最高的玩家在 25 秒内发送三颗 Telegram 原生骰子；每颗有效骰子都会回复识别点数和时间，最多回复前三颗。超时由机器人补发；无人下注时机器人直接掷骰，不等待玩家。
5. 一条最终结果图片消息：上半部分为最近走势，下半部分为全部玩家投注额、返还额、净输赢和最终余额；caption 包含开奖结果并提及本期全部下注玩家。

封盘清单过长时才会自动生成图片，不截断玩家。最终开奖结果与全员结算不再拆成两条消息。走势图采用最多最近
84 期的横屏滚动窗口，每行 14 期，格内不显示超长期号；运行到一万期以后仍只查询和绘制最近窗口，
不会把全部历史期次塞进一张图。群消息使用稳定的 32 位公开奖期代码，管理员可设置最近 14 至 84 期。

执行 `bash scripts/linux/update.sh` 时会先构建镜像和执行迁移，再等待当前下注窗口结束后切换服务。
下注、钱包和期次状态都保存在 PostgreSQL 中，正常更新不会在开放下注阶段中断玩家下单。
